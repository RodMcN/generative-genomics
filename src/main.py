...
class Pipeline:
    def __init__(self, config: Union[Dict, str, Path]):
        ...
        if any(self.output_dir.iterdir()):
            logger.warning(f"Output directory {self.output_dir} is not empty. Data will be overwritten.")
    
    def download(self) -> Path:
        # dataset_name = self.config["dataset"]["name"]
        # output_file = Path(f"/workdir/{dataset_name}.project.h5ad")
        output_file = Path(f"/nvme1/ml_data/rna/E-ANND-5.project.h5ad")
    ...
    def eval_xgb(self, model: "XGBWrapper"):
        ...
        out_file = self.output_dir / f"{self.dataset_name}_{model.name}_real_eval_xgb.tsv"
    ...
    def shap_explain(self, model, cell_type, filename):
        ...
        indices_of_interest = [i for i, cell_type in enumerate(test_y) if cell_type == cell_type_index]
        ...
        shap.summary_plot(shap_values, test_X, feature_names=[f'PC{i+1}' for i in range(test_X.shape[-1])], max_display=10)
    ...
    def train_vae(self):
        ...
        train_vae_(self.train_X.load_once(), 
    ...
    def run(self):
        ...
        xgb_real = self.train_xgb(self.train_X.load(),
                                  self.train_y.load(),
                                  self.val_X.load(),
                                  self.val_y.load(),
                                  "real_data")
        ...
        self.shap_explain(xgb_real, self.config['shap']['cell_type'], f"shap_{self.dataset_name}_real")
        clear_gpu_memory()
        ...
        self.shap_explain(xgb_vae, self.config['shap']['cell_type'], f"shap_{self.dataset_name}_synth")
        clear_gpu_memory()
