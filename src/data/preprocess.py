...
def preprocess(
    ...
    if subsample:
        ...
        adata = adata[
            adata.obs.sample(frac=subsample, random_state=random_state).index
        ].copy()
        logger.info(f"{adata.shape=}")
    ...
    if apply_mapping:
        ...
        adata.obs["celltype_mapping"] = adata.obs["celltype_annotation"].map(
            cell_type_mapping
        )
    ...
    adata = iqr_filter_mito_and_genes(
        adata,
        cell_type_col=cell_type_col if not apply_mapping else "celltype_mapping",
        mito_col=mito_col,
...
def iqr_filter_mito_and_genes(adata, cell_type_col, mito_col, gene_count_col):
    if "split" not in adata.obs.columns:
        raise ValueError(
            "The 'split' column is missing in adata.obs. Run split_data() first."
        )
    ...
    for cell_type in adata.obs[cell_type_col].unique():
        ...
        genes_mask = (gene_counts_all >= lower_bound_genes) & (
            gene_counts_all <= upper_bound_genes
        )
