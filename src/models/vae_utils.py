...
class Metrics:
    def __init__(self, early_stoping_rounds=5, early_stopping_warmup=5, desc="Train"):
        ...
        self.early_stopping_rounds = early_stoping_rounds
    ...
    def end_epoch(self, log=True):
        ...
        if log:
            logger.info(f"{self.desc}: Epoch {self.epochs} | KL: {kl:.4f} | recon: {recon:.4f} | loss: {loss:.4f}")

    def early_stopping(self, verbose=False):
        if verbose:
            logger.info(f"Epochs epochs_since_best = {self.epochs_since_best}. Early Stopping rounds = {self.early_stoping_rounds}")
        if self.epochs < self.early_stopping_warmup:
            return False
        elif self.epochs_since_best > self.early_stopping_rounds:
            return True
        return False
