...
def train(X_train, y_train, X_val, y_val, config, model_path, pbar=False):
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        opt, max_lr=0.001, total_steps=epochs * 2
    )
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y) in tqdm(
            enumerate(train_dataloader, 1), desc="Training", disable=not pbar
        ):
            opt.zero_grad()
        ...
        with torch.no_grad():
            for i, (x, y) in tqdm(
                enumerate(val_dataloader, 1), desc="Eval", disable=not pbar
            ):
                x, y = x.to(device), y.to(device)
                # recon_mu, recon_logvar, q = model(x, y)
                reconstructed, mu, logvar = model(x, y)
                ...
                loss, recon_loss, kl_loss = vae_loss(
                    reconstructed, x, mu, logvar, beta=b
                )
